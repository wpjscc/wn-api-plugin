<?php namespace Wpjscc\Api\Widgets;
...
class Lists extends WidgetBase
{
    ...
    public function onRefresh()
    {
        ...
        $data = [
            ...
            'total' => $this->vars['records']->total(),
            'current_page' => $this->vars['records']->currentPage(),
            'is_last_page' => $this->vars['records']->onLastPage(),
            'per_page' => $this->vars['records']->perPage(),
            'data' => $this->getTransformRecords(),
            'columns' => $this->getVisibleColumns(),
            'search_handler' => $this->searchWidget ? $this->searchWidget->alias.'::onSubmit' : '',
            'search_name' => $this->searchWidget ? implode('.', \Winter\Storm\Html\Helper::nameToArray($this->searchWidget->getName())) : '',
            'api' => [
                'create' => [
                    'url' => '/pages/form/create',
                    'params' => [
                        'api' => ApiController::$apiControllerPrefix.'/create',
        ...
        if ($event = $this->fireSystemEvent('backend.list.extendData', [$data])) {
            $data = $event;
        }
        return $data;
    ...
    protected function getTransformRecords()
    {
        $data = [];
        foreach ($this->vars['records'] as $record) {
            $item = [];
            foreach ($this->vars['columns'] as $column) {
                $item[$column->columnName] = $this->getColumnValue($record, $column);
            }
            $data[] = $item;
        }
    ...
    public function getVisibleColumns()
    {
        ...
        foreach ($columns as $key => $column) {
            foreach ([
                ...
                if ($column->{$value}) {
                    $column->{$value} = trans($column->{$value});
                }
            }
        }
    ...
    public function addColumns(array $columns)
    {
        ...
        foreach ($columns as $columnName => $config) {
            ...
            if (!empty($permissions)) {
                if (!BackendAuth::getUser() || !BackendAuth::getUser()->hasAccess($permissions, false)) {
                    continue;
                }
            }
    ...
    protected function evalCustomListType($type, $record, $column, $value)
    {
        ...
        foreach ($plugins as $availableTypes) {
            ...
            if (is_callable($callback)) {
                return call_user_func_array($callback, [$value, $column, $record, $this->controller]);
            }
        }
