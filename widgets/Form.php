...
use Wpjscc\Api\Classes\FormTabs;
use Wpjscc\Api\Classes\FormField;
...
class Form extends WidgetBase
{
    ...
    public function getTransformRecord()
    {
        $record = [];
        foreach ($this->allFields as $key => $filed) {
            $record[$key] = $filed->value;
        }
        return $record;
    }
        ...
        foreach ($fields as $name => $config) {
            ...
            switch (strtolower($addToArea)) {
                ...
                case FormTabs::SECTION_SECONDARY:
                    ...
                    break;
                default:
                    $this->allTabs->outside->addField($name, $fieldObj, 'default');
                    break;
            }
        }
    }
    ...
    protected function makeFormField($name, $config = [])
    {
        ...
        foreach ([
            ...
            if ($field->{$value}) {
                if ($value == 'comment') {
                    if ($field->commentHtml) {
                        $field->{$value} = trans($field->{$value});
                    } else {
                        $field->{$value} = e(trans($field->{$value}));
                    }
                } else {
                    $field->{$value} = e(trans($field->{$value}));
                }
        ...
        $field->dot_name = $field->arrayName.'.'.$attrName;

        $isArray = $field->config['is_array'] ?? false;
        if ($isArray === true) {
    ...
    {
        $vars['sessionKey'] = $this->getSessionKey();
        $vars['outsideTabs'] = $this->allTabs->outside;
        $vars['primaryTabs'] = $this->allTabs->primary;
        $vars['secondaryTabs'] = $this->allTabs->secondary;
        return $vars;
    }
