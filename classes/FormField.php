...
class FormField
{
    ...
    protected function getFieldNameFromData($fieldName, $data, $default = null)
    {
        ...
        foreach ($keyParts as $key) {
            if ($result instanceof Model && $result->hasRelation($key)) {
                if ($key == $lastField) {
                    $relationType =  $result->getRelationType($key);

                    if ($relationType == 'attachOne') {
                        $result = $this->getAttachOneResult($result->$key(), $key);
                    } elseif ($relationType == 'attachMany') {
                        $result = $this->getAttachManyResult($result->$key(), $key);
                    } else {
    ...
    public function getAttachOneResult($relation, $relationName)
    {
        $value = null;

        $file = ($sessionKey = $relation->getParent()->sessionKey)
            ? $relation->withDeferred($sessionKey)->first()
            : $relation->getParent()->$relationName;
        ...
        $value->name = $value->disk_name;
        $value->extname = $value->extension;
        return [$value];
    }

    public function getAttachManyResult($relation, $relationName)
    {
        $value = null;

        $files = ($sessionKey = $relation->getParent()->sessionKey)
            ? $relation->withDeferred($sessionKey)->get()
            : $relation->getParent()->$relationName;
