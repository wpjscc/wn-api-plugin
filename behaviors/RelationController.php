<?php namespace Wpjscc\Api\Behaviors;
...
class RelationController extends ControllerBehavior
{
    ...
    public function relationRender($field, $options = [])
    {
        ...
        $this->prepareVars();
        // dd($this->viewWidget);
        return $this->viewWidget->onRefresh();
    ...
    public function relationRefresh($field = null)
    {
        ...
        $result = $this->relationRenderView($field);
    ...
    protected function makeViewWidget()
    {
        ...
        if ($this->viewMode === 'multi') {
            ...
            $config->showSearch = $this->getConfig('view[showSearch]', !$this->readOnly);
            ...
            $widget = $this->makeWidget('Wpjscc\Api\Widgets\Lists', $config);
            $widget->bindEvent('list.extendQuery', function ($query)  {
                $this->controller->listExtendRelationQuery($query, $this->field);
            });
            /*
    ...
    protected function makeManageWidget()
    {
        ...
        if ($this->manageMode === 'list' || $this->manageMode === 'pivot') {
            ...
            $config->showSearch = $this->getConfig('manage[showSearch]', !$isPivot);
            ...
            if ($this->searchWidget) {
                ...
                $widget->searchWidget = $this->searchWidget;
            }
        ...
        elseif ($this->manageMode === 'form') {
            ...
            $widget = $this->makeWidget('Wpjscc\Api\Widgets\Form', $config);
        }
    ...
    public function onRelationManageForm()
    {
        ...
        if ($this->manageMode == 'form') {
            if ($this->manageId) {
                return $this->manageWidget->render(['preview' => $this->readOnly])+[
                    'right_options' => [
                        [
                            'text'=>'更新',
                            'type' => 'method',
                            'method' => [
                                'method' => 'onRelationManageUpdate',
                                'debounce' => 500
                            ]
                        ]
                    ]
                ...
                return $this->manageWidget->render()+[
                    'right_options' => [
                        [
                            'text'=>'创建',
                            'type' => 'method',
                            'method' => [
                                'method' => 'onRelationManageCreate',
                                'debounce' => 500
                            ]
                        ]
                    ]
            ...
            return $this->manageWidget->onRefresh();
        }
     ...
     * @return array|null
     */
    public function evalToolbarButtons()
    {
     ...
     */
    public function listExtendRelationQuery($query, $field = null)
    {
    }
}
