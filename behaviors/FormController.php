...
class FormController extends ControllerBehavior
{
    ...
    public function initForm($model, $context = null)
    {
         ...
         */
        $this->formWidget = $this->makeWidget('Wpjscc\Api\Widgets\Form', $config);
         ...
         */
        if ($this->controller->isClassExtendedWith(\Wpjscc\Api\Behaviors\RelationController::class)) {
                ...
                'left_options' => [
                    [
                        ...
                        'route' => [
                            ...
                            'params' => ApiController::$options ?: new stdClass,
                        ]
                    ]
                ],
                'right_options' => [
    ...
    public function create_onSave($context = null)
    {
        ...
        // 创建之后，返回更新数据
        $this->update($this->formGetModel()->id);
        // Flash::success($this->getLang("{$this->context}[flashSave]", 'backend::lang.form.create_success'));
        Flash::success('保存成功');
        
        return $this->formRender()+(post('redirect')?[
            'redirect' => [
                'timeout' => 500,
                'type' => 'redirectTo',
                'url' => '/pages/form/update',
                'params' => [
                    'api' => ApiController::$apiControllerPrefix.'/update/'.$this->formGetModel()->id,
                    'ui' => ApiController::$ui,
                    'a' => 'b'
                ]
            ]
    ...
    {
        $this->create();
        return $this->controller->relationRender(post('_relation_field'));
    }
    ...
    public function update_onSave($recordId = null, $context = null)
    {
        ...
        Flash::success('保存成功');
        return $this->update($recordId, $context);
        if ($redirect = $this->makeRedirect($this->context, $model)) {
            return $redirect;
        }
    }
    ...
    public function formFindModelObject($recordId)
    {
        ...
        if (!$result) {
            throw new \Wpjscc\Api\Exceptions\NotFoundHttpException($this->getLang('not-found-message', 'backend::lang.form.not_found', [
